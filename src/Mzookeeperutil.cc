#include "Mzookeeperutil.h"
#include "Mrpcapplication.h"
#include <mutex>
#include "MrpcLogger.h"
#include <condition_variable>

std::mutex cv_mutex;
std::condition_variable cv;
bool is_connected=false;

// 全局的watcher观察器
void global_watcher(zhandle_t *zh,int type,int status,const char* path,void *watcherCtx)
{
    if(type == ZOO_SESSION_EVENT){
        if(status == ZOO_CONNECTED_STATE){
            std::lock_guard<std::mutex> lock(cv_mutex);
            is_connected=true;
        }
    }
    cv.notify_all();
}

ZkClient::ZkClient()
    : m_zhandle(nullptr)
{

}

ZkClient::~ZkClient()
{
    if(m_zhandle!=nullptr){
        zookeeper_close(m_zhandle);
    }
}

void ZkClient::Start()
{
    // 1.建立rpc和zookeeper的连接
    std::string host = MrpcApplication::getInstance().getConfig().load("zookeeper_ip");
    std::string port = MrpcApplication::getInstance().getConfig().load("zookeeper_port");
    std::string constr = host + ":" + port;

    m_zhandle = zookeeper_init(constr.c_str(),global_watcher,3000,nullptr,nullptr,0);
    if(m_zhandle==nullptr){
        MrpcLogger::ERROR("zookeeper_init error!");
        exit(EXIT_FAILURE);
    }
    // 2.通过watcher监测连接
    std::unique_lock<std::mutex> lock(cv_mutex);
    cv.wait(lock,[]{return is_connected;});
    MrpcLogger::INFO("zookeeper_init success!");
}

void ZkClient::Create(const char* path,const char *data,int datalen,int state)
{
    // 创建znode节点
    char path_buffer[128];
    int bufferlen = sizeof(path_buffer);
    int flag = zoo_exists(m_zhandle,path,0,nullptr);
    // 1.判断节点是否存在
    if(flag == ZNONODE){
        // 2.不存在创建znode节点
        flag = zoo_create(m_zhandle,path,data,datalen,&ZOO_OPEN_ACL_UNSAFE,state,path_buffer,bufferlen);
        // 3.是否创建成功
        if(flag==ZOK){
            LOG(INFO)<<"znode create success... path:"<<path;
        }else{
            LOG(ERROR)<<"znode create success... path:"<<path;
            exit(EXIT_FAILURE);
        }
    }
}

// 获取节点上的数据
std::string ZkClient::GetData(const char* path)
{   
    char buf[64];
    int bufferlen = sizeof(buf);
    int flag = zoo_get(m_zhandle,path,0,buf,&bufferlen,nullptr);
    if(flag!=ZOK){
        MrpcLogger::ERROR("zoo_get error");
        return "";
    }else{
        return buf;
    }
    return "";
}